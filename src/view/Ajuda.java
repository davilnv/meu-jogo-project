	...
	public Ajuda() {
		...
		JPanel panel = new JPanel() {
			protected void paintComponent(Graphics g) {
				...
				g.drawString("O objetivo do jogo é ensinar de uma forma dinâmica as operações matemática.", 20, 130);
				g.drawString("Os números podem ser inimigos ou o resultado da operação. A operação é mostrado", 20, 150);
				g.drawString("na lousa e o jogador precisa encostar apenas no que ele considera ser o resultado", 20, 170);
				g.drawString("da operação, perdendo vida quando encostar em um número errado. Para a segunda", 20, 190);
				g.drawString("fase precisa conseguir 15 pontos, ganha quando conseguir 30 pontos. Para o", 20, 210);
				g.drawString("multplayer funciona da mesma forma, ganha quem conseguir a pontuação primeiro.", 20, 230);

				g.drawString("- Status do jogador:", 20, 250);
				g.drawString("    -> Somando: O jogador está com vida e pode continuar acumulando pontos;", 20, 270);
				g.drawString("    -> Game Over: O jogador chegou sua vida a zero e o jogo finaliza, para reiniciar", 20, 290);
				g.drawString("       apertar o botão sair;", 20, 310);
				g.drawString("    -> Ganhou: O jogador conseguiu alcançar 30 pontos e conseguiu ganhar o jogo, ", 20, 330);
				g.drawString("       para reiniciar apertar o botão sair", 20, 350);
